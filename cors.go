...
type Options struct {
	...
	Debug bool
	// if true, disable canonical header
	DisableCanonicalHeader bool
}
...
func New(options Options) *Cors {
	c := &Cors{
		...
		disableCanonicalHeader: options.DisableCanonicalHeader,
	}
	...
	if len(options.AllowedHeaders) == 0 {
		...
		cvt := http.CanonicalHeaderKey
		if options.DisableCanonicalHeader {
			cvt = transparentConverter()
		}

		c.allowedHeaders = convert(append(options.AllowedHeaders, "Origin"), cvt)
...
func nonCors(r *http.Request) bool {
